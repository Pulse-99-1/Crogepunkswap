...
export default function Manage({
  ...
  console.log(id)
  const stakingInfos = useStakingInfo(stakingTokenPair, tokenA, id != null ? Number(id) : null)
  ...
  const photon = chainId ? UNI[chainId] : undefined
  const photonPriceUnParsed = useUSDCPrice(photon)
  const strPhotonPrice = photonPriceUnParsed ? `${photonPriceUnParsed.toFixed()}` : `1`;
  const photonPrice = parseFloat(strPhotonPrice)
  const photonPriceMul1000 = photonPrice * 1000000
  const singleStakingPrice = parseFloat(JSBI.divide(JSBI.multiply(stakingInfo?.totalStakedAmount?.raw ?? JSBI.BigInt(0), JSBI.BigInt(photonPriceMul1000.toFixed())), JSBI.BigInt(10 ** 24)).toString())

  const valueOfTotalStakedAmountInUSDC = singleStakingPrice
    ...
    <PageWrapper gap="lg" justify="center">
      ...
      <PositionInfo gap="lg" justify="center" dim={showAddLiquidityButton}>
        ...
        {!userLiquidityUnstaked ? null : userLiquidityUnstaked.equalTo('0') ? null : (
          <TYPE.main>{userLiquidityUnstaked.toSignificant(6)} {(currencyA?.getDisplaySymbol(chainId))} tokens available</TYPE.main>
        )}
      </PositionInfo>
    </PageWrapper>
  )
