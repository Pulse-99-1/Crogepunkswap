...
import { ChainId, JSBI } from "@photonswap/sdk";
...
import { BIG_INT_ZERO, UNI } from "../../constants";
...
export default function ManageSyrup({
  ...
  let syrupInfo = syrupInfos?.reduce<any>((memo, staking) => {
    if (
      staking.stakingRewardAddress.toLowerCase() ===
      rewardsAddress.toLowerCase()
    ) {
      return staking;
    } else {
      return memo;
  ...
  const showAddLiquidityButton = Boolean(
    syrupInfo?.stakedAmount?.equalTo("0") && userLiquidityUnstaked?.equalTo("0")
  );

  // fade cards if nothing staked or nothing earned yet
  const disableTop =
    !syrupInfo?.stakedAmount || syrupInfo.stakedAmount.equalTo(JSBI.BigInt(0));

  const backgroundColor = useColor(tokenA);

  // get WETH value of staked LP tokens
  const fixedPlaces =
    syrupInfo?.earnedAmount.token.decimals < 6
      ? syrupInfo?.earnedAmount.token.decimals
      : 6;
  ...
  const USDPrice = useUSDCPrice(
    UNI[chainId === undefined ? ChainId.CRONOSMAINNET : chainId]
  );
  //@ts-ignore
  const valueOfTotalStakedAmountInUSDC =
    syrupInfo?.totalStakedAmount.toSignificant(6) *
    syrupInfo?.dQUICKtoQUICK.toSignificant(6) *
    USDPrice?.toSignificant(6);

  //@ts-ignore
  const valueOfMyStakedAmountInUSDC =
    syrupInfo?.stakedAmount.toSignificant(6) *
    syrupInfo?.dQUICKtoQUICK.toSignificant(6) *
    USDPrice?.toSignificant(6);
    ...
    <PageWrapper gap="lg" justify="center">
            ...
            <AutoColumn gap="md">
              <RowBetween>
                <TYPE.white fontWeight={600}>
                  Please get CROGEPUNK tokens to participate
                </TYPE.white>
              </RowBetween>
            </AutoColumn>
          </CardSection>
      ...
      <PositionInfo gap="lg" justify="center" dim={showAddLiquidityButton}>
                ...
                <RowBetween style={{ alignItems: "baseline" }}>
                  <TYPE.white fontSize={36} fontWeight={600}>
                    {valueOfMyStakedAmountInUSDC
                      ...
                      : `${
                          syrupInfo?.stakedAmount?.toSignificant(4, {
                            groupSeparator: ",",
                          }) ?? "-"
                        }`}
                  </TYPE.white>
                  <TYPE.white>dQUICK</TYPE.white>
                </RowBetween>
              </AutoColumn>
            </CardSection>
          </StyledDataCard>
          <StyledBottomCard
            dim={syrupInfo?.stakedAmount?.equalTo(JSBI.BigInt(0))}
          >
            <CardBGImage desaturate />
            <CardNoise />
            <AutoColumn gap="sm">
              <RowBetween>
                <div>
                  <TYPE.black>
                    Your unclaimed {currency?.symbol ? currency?.symbol : ""}
                  </TYPE.black>
                </div>
                {syrupInfo?.earnedAmount &&
                  JSBI.notEqual(BIG_INT_ZERO, syrupInfo?.earnedAmount?.raw) && (
                    <ButtonEmpty
                      padding="8px"
                      borderRadius="8px"
                      width="fit-content"
        ...
        <TYPE.main style={{ textAlign: "center" }} fontSize={14}>
          ...
          {currency?.symbol ? currency?.symbol : ""} on your behalf!
        </TYPE.main>
        <TYPE.main
          style={{ textAlign: "center", marginTop: -12 }}
          fontSize={14}
        >
          * Incentivised rewards are not an endorsement and you are strongly
          encouraged to DYOR when providing liquidity.
        </TYPE.main>
            ...
            {!syrupInfo?.ended && (
              ...
              >
                {syrupInfo?.stakedAmount?.greaterThan(JSBI.BigInt(0))
                  ? "Deposit"
                  : syrupInfo && syrupInfo?.name !== ""
                  ? "Deposit " + syrupInfo?.name + " Tokens"
                  : "Deposit dQUICK"}
              </ButtonPrimary>
            )}
        ...
        {!userLiquidityUnstaked ? null : userLiquidityUnstaked.equalTo(
          ...
          <TYPE.main>
            ...
            {syrupInfo?.name !== "" ? syrupInfo?.name : "dQUICK"} tokens
            available
          </TYPE.main>
        )}
      </PositionInfo>
