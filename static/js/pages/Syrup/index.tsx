...
export default function Syrup() {
  ...
  var poolsToShow = syrupInfos.filter((syrupInfo) => {
    if (syrupInfo.token.symbol && syrupInfo.token.name) {
      return (
        ...
        syrupInfo.token.name.toLowerCase().indexOf(searchQuery.toLowerCase()) >
          -1
      );
  ...
  //@ts-ignore
  const maxPage =
    poolsToShow.length <= 10 ? 1 : Math.ceil(poolsToShow.length / 10);
  ...
  const stakingRewardsExist = Boolean(
    typeof chainId === "number" &&
      (SYRUP_REWARDS_INFO[chainId]?.length ?? 0) > 0
  );

  poolsToShow?.sort((a, b) => {
    if (
      Boolean(a.stakedAmount.greaterThan("0")) &&
      Boolean(b.stakedAmount.greaterThan("0"))
    ) {
      return 1;
    }
    if (
  ...
  const poolsWithData = filteredPools.map((syrupInfo, index) => {
    ...
    const priceOfRewardTokenInUSD =
      price?.toSignificant(6) * USDPriceBaseToken?.toSignificant(6);
    let rewards = 0;
    //@ts-ignore
    rewards =
      syrupInfo?.rate * (priceOfRewardTokenInUSD ? priceOfRewardTokenInUSD : 0);
    ...
    if (syrupInfo?.valueOfTotalStakedAmountInUSDC > 0) {
      //@ts-ignore
      tokenAPR =
        (rewards / syrupInfo?.valueOfTotalStakedAmountInUSDC) * 365 * 100;
    }
    ...
    return {
      ...
      depositValue:
        ...
        syrupInfo?.totalStakedAmount?.toSignificant(),
    };
  });
  ...
  useEffect(() => {
    if (syrupInfos.length > 0) {
      var aTotaldepositsUSD: any = syrupInfos?.reduce(
        (sum, current, currentIndex) => {
          if (currentIndex === 1)
            //@ts-ignore
            return (
              sum.valueOfTotalStakedAmountInUSDC +
              current.valueOfTotalStakedAmountInUSDC
            );
          //@ts-ignore
          else return sum + current.valueOfTotalStakedAmountInUSDC;
        }
      );
            ...
            <AutoColumn gap="md">
              ...
              <RowBetween>
                <TYPE.white fontSize={14}>
                  Deposit your CROGEPUNK tokens to earn more.
                </TYPE.white>
              </RowBetween>{" "}
              <RowBetween>
                <ExternalLink
                  id={`old-pools-link`}
                  href={"https://quickswap.exchange/#/archive"}
          ...
          {stakingRewardsExist && poolsToShow?.length === 0 ? (
            ...
            refinedPools
              ?.slice(
                ...
                page * ITEMS_PER_PAGE < poolsToShow.length
                  ? page * ITEMS_PER_PAGE
                  : poolsToShow.length
              )
              .map((syrupInfo) => {
                ...
                return (
                  <SyrupCard
                    key={syrupInfo.stakingRewardAddress}
                    syrupInfo={syrupInfo}
                    isOld={false}
                  />
                );
